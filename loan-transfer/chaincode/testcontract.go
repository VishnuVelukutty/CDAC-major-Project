...
type ApproverGroup struct {
	ApproverName	bool	`json:"ApproverName"`
...
func (t *TestContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	loanRequests = []LoanRequest{
		...
		},
		{
			...
			ApprovedBy: {ApproverName: "Axis", LoanOffered: "50 lakhs"}
		},
		{
			RequestID: "request3", 
			RequesterName: "Jane", 
			...
			Consent: true,
			RegulatoryCheck: true,
			ApprovedBy: {ApproverName: "Hdfc", LoanOffered: "1 crore"}
		},
		{
			RequestID: "request4", 
			RequesterName: "David", 
			...
			Consent: true,
			RegulatoryCheck: true,
			ApprovedBy: {ApproverName: "Axis", LoanOffered: "3 crore"}
		},
		{
			RequestID: "request5", 
			RequesterName: "Smith", 
			...
			Consent: true,
			RegulatoryCheck: true,
			ApprovedBy: {ApproverName: "Sbi", LoanOffered: "5 lakhs"}
		}
	}
...
func (t *TestContract) CreateLoanRequest(ctx contractapi.TransactionContextInterface id,
requestId string, requesterName string, loanAmount string, 
interestRate float64, noOfYears int, repaymentMode string) error {
...
func (t *TestContract) RegulationCheck(ctx contractapi.TransactionContextInterface, requestId string) error{
	...
	if mspid == "RegulatorMSP" {
		...
		fmt.Errorf("Only Regulator can approve regulation checks.")
	}
}
...
func (t *TestContract) LoanApprove(ctx contractapi.TransactionContextInterface requestId string) (bool, error) {
	...
	if mspid == "AxisMSP"{
		...
		fmt.Errorf("Only Approver Group can call this function")
	}
}
...
func (t *TestContract) LoanDusburse(ctx contractapi.TransactionContextInterface, requestId string) error {
	...
	exists, err := t.RequestExists(ctx, requestId)
		...
		if loanRequest.Consent {
			...
			fmt.Errorf("Loan cannot be disbursed as consent is not provided")
		}
}
