...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0requestor_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0axis_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ${CONTAINER_CLI} run  --rm -d --name peer0hdfc_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ${CONTAINER_CLI} run  --rm -d --name peer0sbi_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ${CONTAINER_CLI} run  --rm -d --name peer0regulator_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0requestor_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0axis_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0hdfc_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0sbi_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0regulator_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
infoln "Installing chaincode on peer0.requestor..."
installChaincode "requestor"
infoln "Install chaincode on peer0.axis..."
installChaincode "axis"
infoln "Install chaincode on peer0.hdfc..."
installChaincode "hdfc"
infoln "Install chaincode on peer0.sbi..."
installChaincode "sbi"
infoln "Install chaincode on peer0.regulator..."
installChaincode "regulator"

## query whether the chaincode is installed
queryInstalled "requestor"

## approve the definition for org1
approveForMyOrg "requestor"

## check whether the chaincode definition is ready to be committed
## expect org1 to have approved and org2 not to
checkCommitReadiness "requestor" "\"RequestorMSP\": true" "\"AxisMSP\": false" "\"HdfcMSP\": false" "\"SbiMSP\": false" "\"RegulatorMSP\": false"

## now approve also for org2
approveForMyOrg "axis"

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness "axis" "\"RequestorMSP\": true" "\"AxisMSP\": true" "\"HdfcMSP\": false" "\"SbiMSP\": false" "\"RegulatorMSP\": false"

## now approve also for org2
approveForMyOrg "hdfc"

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness "hdfc" "\"RequestorMSP\": true" "\"AxisMSP\": true" "\"HdfcMSP\": true" "\"SbiMSP\": false" "\"RegulatorMSP\": false"
...
## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness "sbi" "\"RequestorMSP\": true" "\"AxisMSP\": true" "\"HdfcMSP\": true" "\"SbiMSP\": true" "\"RegulatorMSP\": false"
...
## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness "regulator" "\"RequestorMSP\": true" "\"AxisMSP\": true" "\"HdfcMSP\": true" "\"SbiMSP\": true" "\"RegulatorMSP\": true"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition "requestor" "axis" "hdfc" "sbi" "regulator"

## query on both orgs to see that the definition committed successfully
queryCommitted "requestor"
queryCommitted "axis"
queryCommitted "hdfc"
queryCommitted "sbi"
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit "requestor" "axis" "hdfc" "sbi" "regulator"
fi
