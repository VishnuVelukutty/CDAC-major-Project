...
# use certiforg4icates and keys to sign and verify its actions. In addition, each user
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Requestor Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-requestor.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Axis Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-axis.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/requestor/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Requestor, Axis, Hdfc, Sbi, Regulator"
...
# genesis block is defined using the "FiveOrgsApplicationGenesis" profile at the bottom
...
# call the script to create the channel, join the peers of requestor and axis,
...
function networkDown() {
  ...
  # stop hdfc corg4ontainers also in addition to requestor and axis, in case we were running sample to add hdfc
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.banking.com docker_peer0.requestor.banking.com docker_peer0.axis.banking.com docker_peer0.hdfc.banking.com docker_peer0.sbi.banking.com docker_peer0.regulator.banking.com 
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/requestor/msp organizations/fabric-ca/requestor/tls-cert.pem organizations/fabric-ca/requestor/ca-cert.pem organizations/fabric-ca/requestor/IssuerPublicKey organizations/fabric-ca/requestor/IssuerRevocationPublicKey organizations/fabric-ca/requestor/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/axis/msp organizations/fabric-ca/axis/tls-cert.pem organizations/fabric-ca/axis/ca-cert.pem organizations/fabric-ca/axis/IssuerPublicKey organizations/fabric-ca/axis/IssuerRevocationPublicKey organizations/fabric-ca/axis/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hdfc/msp organizations/fabric-ca/hdfc/tls-cert.pem organizations/fabric-ca/hdfc/ca-cert.pem organizations/fabric-ca/hdfc/IssuerPublicKey organizations/fabric-ca/hdfc/IssuerRevocationPublicKey organizations/fabric-ca/hdfc/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/sbi/msp organizations/fabric-ca/sbi/tls-cert.pem organizations/fabric-ca/sbi/ca-cert.pem organizations/fabric-ca/sbi/IssuerPublicKey organizations/fabric-ca/sbi/IssuerRevocationPublicKey organizations/fabric-ca/sbi/fabric-ca-server.db'
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addOrg3/fabric-ca/hdfc/msp addOrg3/fabric-ca/hdfc/tls-cert.pem addOrg3/fabric-ca/hdfc/ca-cert.pem addOrg3/fabric-ca/hdfc/IssuerPublicKey addOrg3/fabric-ca/hdfc/IssuerRevocationPublicKey addOrg3/fabric-ca/hdfc/fabric-ca-server.db'
...
# use this as the default docker-compose yaml definition for hdfc
COMPOSE_FILE_HDFC_BASE=compose-hdfc.yaml
# use this as the docker compose couch file for hdfc
COMPOSE_FILE_HDFC_COUCH=compose-couch-hdfc.yaml
# certificate authorities compose file
COMPOSE_FILE_HDFC_CA=compose-ca-hdfc.yaml
#
