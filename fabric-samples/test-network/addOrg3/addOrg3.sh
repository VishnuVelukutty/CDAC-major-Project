...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.banking.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.banking.com/peers/peer0.org3.banking.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.banking.com" ]; then
    generateOrg3
